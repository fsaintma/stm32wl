...
void LoRaWAN_Init(void)
{
  /* USER CODE BEGIN LoRaWAN_Init_1 */
    ...
    if (ADMIN_DEFAULT_CLASS == CLASS_A){
    	APP_LOG(0, 1, "> LoRaWAN Class:          CLASS A \r\n");
    }
    else if (ADMIN_DEFAULT_CLASS == CLASS_B){
    	APP_LOG(0, 1, "> LoRaWAN Class:          CLASS B \r\n");
    }
    else if (ADMIN_DEFAULT_CLASS == CLASS_C){
    	APP_LOG(0, 1, "> LoRaWAN Class:          CLASS C \r\n");
    }
    else {
    	APP_LOG(0, 1, "> LoRaWAN Class:          ERROR (check your initial setup) \r\n");
    }
    ...
    if (appData->Port == 0){
		...
		if(appData->BufferSize>0)
		{
			for(int i=0;i<appData->BufferSize;i++){
			  APP_LOG(TS_OFF, VLEVEL_L, "%02x ", appData->Buffer[i]);
			}
		}
