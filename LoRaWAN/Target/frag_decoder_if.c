...
#include "lora_app.h" // aded for SendTXStopFrag
...
/**
  ...
  */
static uint32_t Crc32_(uint8_t *buffer, uint16_t length);
...
void FRAG_DECODER_IF_OnProgress(uint16_t fragCounter, uint16_t fragNb, uint8_t fragSize, uint16_t fragNbLost)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "RECEIVED    : %5d / %5d Fragments\r\n", fragCounter, fragNb);
  APP_LOG(TS_OFF, VLEVEL_L, "              %5d / %5d Bytes\r\n", fragCounter * fragSize, fragNb * fragSize);
  APP_LOG(TS_OFF, VLEVEL_L, "LOST        :       %7d Fragments\r\n\r\n", fragNbLost);
...
#if (INTEROP_TEST_MODE == 1)
void FRAG_DECODER_IF_OnDone_DataDisplay (void){

	  APP_LOG(TS_OFF, VLEVEL_L, "Data received  : \r\n");
	  for (int i=0 ; i<FRAG_MAX_NB ; i++ ){
		  APP_LOG(TS_OFF, VLEVEL_L, "%02x ", UnfragmentedData[i]);
		  //if (i < FRAG_MAX_SIZE){
		//	  APP_LOG(TS_OFF, VLEVEL_L, "\n");
		  //}
	  }
...
void FRAG_DECODER_IF_OnDone(int32_t status, uint32_t size)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "STATUS         : %d\r\n", status);

#if (INTEROP_TEST_MODE == 0)
  /* Do a request to Run the Secure boot - The file is already in flash */
  // FwUpdateAgent_Run();
#else
  uint32_t FileRxCrc = Crc32_(UnfragmentedData, size);
  APP_LOG(TS_OFF, VLEVEL_L, "Size           : %d\r\n", size);
  APP_LOG(TS_OFF, VLEVEL_L, "CRC            : %08X\r\n\r\n", FileRxCrc);
  BSP_LED_Off(LED_BLUE);

  FRAG_DECODER_IF_OnDone_DataDisplay();

  	APP_LOG(TS_OFF, VLEVEL_L, "\r\nData Block has been completely received (No need of further fragments).\r\n" );
  	APP_LOG(TS_OFF, VLEVEL_L,"Contact the Server to signal it (Applicative uplink: 0x0001 on Fport 186)\r\n -->\r\n");
	SendTXStopFrag();
